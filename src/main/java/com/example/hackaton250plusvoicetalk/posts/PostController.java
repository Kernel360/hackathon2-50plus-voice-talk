package com.example.hackaton250plusvoicetalk.posts;
...
public class PostController {
    ...
    @CrossOrigin(origins = "http://10.230.110.50:3000")
    @PostMapping("/{user_id}")
    public Api<PostResponse> create(@RequestBody Api<PostRequest> postDto, HttpServletRequest request, @PathVariable Long user_id) {
/*        Long userId = authenticated(request);
        if(userId==null) {
            return Api.buildApiResponse(HttpStatus.BAD_REQUEST,null);
        }*/
        return Api.buildApiResponse(HttpStatus.OK, postService.save(postDto.getData(),user_id));
    } //pathvariable로 바꿈.. 에러나와서

    @CrossOrigin(origins = "http://10.230.110.50:3000")
    @PatchMapping("/{postId}/{user_id}")
    public Api<PostResponse> delete(@PathVariable Long postId, @PathVariable Long user_id, HttpServletRequest request) {
/*        Long userId = authenticated(request);
        ...
        Optional<PostResponse> target = postService.delete(postId, user_id);
...
/*        Long userId = authenticated(request);
        ...
        Optional<PostResponse> target = postService.update(postDto.getData(), postId, user_id);
        return target.map(postResponse -> buildApiResponse(HttpStatus.OK, postResponse))
                .orElseGet(() -> buildApiResponse(HttpStatus.BAD_REQUEST, null));
    }
